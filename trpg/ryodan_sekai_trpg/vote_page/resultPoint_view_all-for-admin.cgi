...
if($ENV{'REMOTE_HOST'} !~ /zaq/){
	print "違法接続です、ホストアドレスを記録させていただきました。";
	exit;
}else{
}
...
foreach $id (@IDlist){
	...
	foreach $com (@comlist) {
		...
		for( $i = 10 ; $i<99; $i += 2){
			...
			$pairs[$j] =~ s/&gt;/>/g;
			#$pairs[$j] =~ s/<BR>/\n/g;# 改行コードを戻す
			$clistcom[$pairs[$i]] .= "●[$pairs[0]]".$pairs[$j]."<BR>";
	...
	for( $i = 0 ; $i<99; $i += 1){
		if($clist[$i] ne ""){
			print "<B>$clist[$i]</B>へのコメント（$pointlist[$i]点）<BR>".$clistcom[$i];
			print "<BR><HR><BR>";
		}
	}
}
